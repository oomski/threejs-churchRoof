...
const churchRoofGlb = await gltfLoader.loadAsync(
  `${import.meta.env.BASE_URL}Day 12 - ChurchRoof.glb`
);
const churchRoof = churchRoofGlb.scene;

churchRoof.traverse((child) => {
  if (child.isMesh) {
...
churchRoof.updateMatrixWorld(true);

// compute bounds and scale so the model's largest dimension equals `targetSize`
let box = new THREE.Box3().setFromObject(churchRoof);
const size = box.getSize(new THREE.Vector3());
const maxDim = Math.max(size.x, size.y, size.z);
const targetSize = 4; // world units you want the model to fit in
if (maxDim > 0) {
  const scale = targetSize / maxDim;
  churchRoof.scale.setScalar(scale);
  churchRoof.updateMatrixWorld(true); // update after scaling
}

// recompute bounds and get center
box = new THREE.Box3().setFromObject(churchRoof);
const center = box.getCenter(new THREE.Vector3());

// create a pivot at the world origin and add the model offset so its center is at pivot
const pivot = new THREE.Group();
scene.add(pivot);
churchRoof.position.sub(center); // move model so its center is at (0,0,0) relative to pivot
